...
class SerialReader():
    def __init__(self, capture_path, date_str, port='/dev/ttyACM0', baudrate=115200, timeout=1, freq=100):
        super().__init__()
        self.port = port
        ...
        self.data = np.zeros((60000, 18), dtype=np.float64)
        ...
        self.exit = Event()
        self.capture_path = os.path.join("/home/temp_id/capture", capture_path, "contact")
    ...
    def run(self):
            ...
            while not self.exit.is_set():
                if self.arduino.readable():
                    ...
                    try:
                        ...
                        self.log.append(f"[WARNING] Invalid data received: {raw_data}")
        ...
        np.save(os.path.join(self.capture_path, f"data_{self.date_str}.npy"), self.data[:self.cnt])
        np.save(os.path.join(self.capture_path, f"timestamp_{self.date_str}.npy"), self.timestamp[:self.cnt])
        
        with open(os.path.join(self.capture_path, f"log_{self.date_str}.txt"), "w") as f:
