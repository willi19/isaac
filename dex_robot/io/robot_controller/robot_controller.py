...
class DexArmControl:
    def __init__(self, capture_path, datetime, xarm_ip_address="192.168.1.221"):
        ...
        self.capture_path = os.path.join("/home/temp_id/capture", capture_path, "robot")
        ...
        self.arm_state_timestamp = np.zeros((60000, 1), dtype=np.float64)
        
        self.arm_action_hist = np.zeros((60000, 6), dtype=np.float64)
        self.arm_action_time_hist = np.zeros((60000, 1), dtype=np.float64)

        self.hand_state_hist = np.zeros((60000, 16), dtype=np.float64)
        self.hand_state_timestamp = np.zeros((60000, 1), dtype=np.float64)
        
        self.hand_action_hist = np.zeros((60000, 16), dtype=np.float64)
        self.hand_action_time_hist = np.zeros((60000, 1), dtype=np.float64)
    ...
    def move_hand(self):
        ...
        while not self.exit.is_set():
            ...
            if self.ready_array[0] == 1: 
                ...
                self.hand_state_timestamp[self.hand_cnt] = start_time

                self.hand_action_hist[self.hand_cnt] = self.hand_target_action_array.copy()
                self.hand_action_time_hist[self.hand_cnt] = start_time
                self.hand_cnt += 1
        ...
        np.save(os.path.join(self.capture_path, f"hand_state_hist_{self.datetime}.npy"), self.hand_state_hist[:self.hand_cnt])
        np.save(os.path.join(self.capture_path, f"hand_state_timestamp_{self.datetime}.npy"), self.hand_state_timestamp[:self.hand_cnt])

        np.save(os.path.join(self.capture_path, f"hand_action_hist_{self.datetime}.npy"), self.hand_action_hist[:self.hand_cnt])
        np.save(os.path.join(self.capture_path, f"hand_action_time_hist_{self.datetime}.npy"), self.hand_action_time_hist[:self.hand_cnt])
    ...
    def move_arm(
        ...
        while not self.exit.is_set():
            ...
            if self.ready_array[0] == 1:
                ...
                self.arm_state_timestamp[self.arm_cnt] = start_time
                
                self.arm_action_hist[self.arm_cnt] = angles.copy()
                self.arm_action_time_hist[self.arm_cnt] = start_time
                self.arm_cnt += 1
        ...
        np.save(os.path.join(self.capture_path, f"arm_state_hist_{self.datetime}.npy"), self.arm_state_hist[:self.arm_cnt])
        np.save(os.path.join(self.capture_path, f"arm_state_timestamp_{self.datetime}.npy"), self.arm_state_timestamp[:self.arm_cnt])

        np.save(os.path.join(self.capture_path, f"arm_action_hist_{self.datetime}.npy"), self.arm_action_hist[:self.arm_cnt])
        np.save(os.path.join(self.capture_path, f"arm_action_time_hist_{self.datetime}.npy"), self.arm_action_time_hist[:self.arm_cnt])
