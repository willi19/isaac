...
class XSensReceiver:
    def __init__(self) -> None:
        ...
        self.shm = {}
        # self.shm["timestamp"] = shared_memory.SharedMemory(create=True, name="timestamp",size=1 * np.dtype(np.float32).itemsize)
        # self.shm["timestamp_recv"] = shared_memory.SharedMemory(create=True, name="timestamp_recv",size=1 * np.dtype(np.float32).itemsize)
        # self.shm["pose_data_recv"] = shared_memory.SharedMemory(create=True, name="pose_data_recv",size=1 * np.dtype(np.float32).itemsize)
        # self.shm["hand_pose"] = shared_memory.SharedMemory(create=True, name="hand_pose",size=1 * np.dtype(np.float32).itemsize)
        # self.shm["hand_joint_angle"] = shared_memory.SharedMemory(create=True, name="hand_pose",size=1 * np.dtype(np.float32).itemsize)
        self.create_shared_memory("timestamp", 1 * np.dtype(np.float32).itemsize)
        self.create_shared_memory("timestamp_recv", 1 * np.dtype(np.float32).itemsize)
        self.create_shared_memory("pose_data_recv", 1 * np.dtype(np.float32).itemsize)
        ...
        self.data_array = {
            "timestamp": np.ndarray((1,), dtype=np.float32, buffer=self.shm["timestamp"].buf),
            "timestamp_recv": np.ndarray((1,), dtype=np.float32, buffer=self.shm["timestamp_recv"].buf),
            "pose_data_recv": np.ndarray((1,), dtype=np.float32, buffer=self.shm["pose_data_recv"].buf),
        ...
        while not self.stop_event.is_set():
            ...
            if message_id == 25:
                ...
                if pose_data_parsed[1] == "bodyhands":
                    ...
                    self.data_array["hand_pose"][:, :, :] = pose_data_parsed[0][23+20:23+40]
                    for i in range(0, 20):
                        self.data_array["hand_pose"][i] = np.linalg.inv(pose_data_parsed[0][0]) @ self.data_array["hand_pose"][i] # pelvis relative
                    
                    self.data_array["hand_joint_angle"][:, :] = self.pose_to_jointangle(
                        pose_data_parsed[0][23+20:23+40], hand_index.hand_index_parent
                    )

                    left_hand_pose = pose_data_parsed[0][23+0:23+20]
                    for i in range(1, 20):
