...
def main():
    ...
    for demo_name in sim_demo_path_list:
        ...
        T = sim_obj_traj.shape[0]
        sim_heights = []
        teleop_heights = []

        for step in range(T):
            # Compute object heights
            h = compute_mesh_to_ground_distance(sim_obj_traj[step], mesh)
            sim_heights.append(h)
            h = compute_mesh_to_ground_distance(teleop_obj_traj[step], mesh) + 0.01
            teleop_heights.append(h)
        sim_max_height = np.max(sim_heights)
        teleop_max_height = np.max(teleop_heights)
        if teleop_max_height > 0.05:
            success += 1
        # else:
        #     plt.plot(sim_heights, label="Simulation")
        #     plt.plot(teleop_heights, label="Teleoperation")
        #     plt.legend()
        #     plt.show()
