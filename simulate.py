...
os.makedirs(f"data/simulation/{obj_name}", exist_ok=True)
...
def get_closest_height(sim, env, actor_handle):
    # Get the mesh geometry of the actor
    mesh_geometry = gym.get_actor_rigid_body_shapes(env, actor_handle)

    closest_distance = float("inf")
    floor_height = 0.0  # Assuming floor is at z = 0. Adjust if necessary

    # Iterate over all vertices in the mesh
    for mesh in mesh_geometry:
        vertices = mesh.vertices
        for vertex in vertices:
            # Convert the vertex to world coordinates
            world_position = (
                gym.get_actor_rigid_body_global_pose(env, actor_handle) @ vertex
            )
            # Calculate the distance to the floor
            distance = abs(world_position.z - floor_height)
...
for demo_name in demo_path_list:
    os.makedirs(f"data/simulation/{obj_name}/{demo_name}", exist_ok=True)
    ...
    sim_obj_traj = {obj_name: np.zeros((T, 4, 4))}
    sim_robot_state = np.zeros((T, 22))
    ...
    for step in range(T):
        ...
        if view_physics:
            ...
            robot_dof_state_pos = gym.get_actor_dof_states(
                env, actor_handle["robot"], gymapi.STATE_POS
            )
            sim_robot_state[step] = robot_dof_state_pos["pos"]
            ...
            sim_obj_traj[obj_name][step] = np.eye(4)
            sim_obj_traj[obj_name][step][:3, :3] = obj_rotmat
            sim_obj_traj[obj_name][step][:3, 3] = obj_pos
    ...
    pickle.dump(
        sim_obj_traj,
        open(f"data/simulation/{obj_name}/{demo_name}/obj_traj.pickle", "wb"),
    )
    np.save(f"data/simulation/{obj_name}/{demo_name}/robot_qpos.npy", sim_robot_state)
