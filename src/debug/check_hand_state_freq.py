...
if __name__ == "__main__":
    ...
    for name in name_list:
        ...
        for index in index_list:
            ...
            robot_action = np.load(os.path.join(root_path, "hand", "state.npy"))
            robot_timestamp = np.load(os.path.join(root_path, "hand", "timestamp.npy"))
            ...
            os.makedirs(f"timediff_hand/{name}", exist_ok=True)
            plt.plot(robot_timestamp[1:] - robot_timestamp[0],fuck, marker='o', linestyle='None')
            plt.savefig(f"timediff_hand/{name}/{index}.png")
            plt.cla()
