...
if __name__ == "__main__":
    ...
    obj_list = os.listdir(os.path.join(home_dir, "shared_data","processed"))
    for obj_name in obj_list:
        source_path = os.path.join(home_dir, "shared_data", "processed", obj_name)
        destination_path = os.path.join(home_dir, "download", "processed", obj_name)

        if os.path.exists(f"video/final/{obj_name}") and len(os.listdir(f"video/final/{obj_name}")) > 0:
            continue
