...
from dex_robot.utils.file_io_prev import load_robot_traj, load_robot_target_traj
import transforms3d
# qpos = np.array(
#     [
#         -1.60879326e00,
...
link_pose = np.array([[0,0,1,0.45],[1,0,0,0],[0,1,0,0.04],[0,0,0,1]])
hand_pose = np.zeros(16)
hand_pose[12] = np.pi/8
hand_pose[13] = np.pi/2

np.save("data/home_pose/contact_test_allegro_eef_frame.npy", link_pose)
np.save("data/home_pose/contact_test_allegro_hand_joint_angle.npy", hand_pose)
# np.save("data/home_pose/contact_test_allegro_robot_action.npy", qpos)
