...
def main():
    ...
    arm_controller = DexArmControl("test/mingi_real", date_time)
    contact_receiver = SerialReader("test/mingi_real", date_time)
    camera_receiver = CameraManager("test/mingi_real", date_time, is_streaming=False, syncMode=False)
    os.makedirs("test/mingi_real", exist_ok=True)

    # xarm_home_pose=homo2cart(home_wrist_pose), allegro_home_pose=home_hand_pose)
